    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4">
          <div>
            <h2 className="text-[18px] font-bold text-content-primary">AI 专家团</h2>
            <p className="text-[12px] text-content-muted mt-0.5">
              ...
              <span className="text-content-icon ml-1">（至少{minSlots}位）</span>
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-surface-page hover:bg-surface-hover flex items-center justify-center active:scale-90 transition-all"
          >
              ...
              return (
                <div key={agent.id} className="relative">
                  ...
                  <button
                    ...
                    disabled={isDisabled}
                    className={`
                      relative p-4 rounded-2xl bg-surface-card text-left transition-all w-full flex flex-col
                      ${isSelected
                        ? 'border-2 border-[#AAE874] shadow-[0_8px_32px_rgba(170,232,116,0.25)]'
                        : 'border border-line shadow-[0_4px_16px_rgba(0,0,0,0.06)]'
                      }
                  ...
                  >
                    ...
                    <div className="relative flex flex-col h-full">
                      ...
                      <div className="space-y-1 flex-1 flex flex-col items-center text-center">
                        <h3 className="text-[14px] text-content-primary font-bold">{agent.name}</h3>
                        {agent.description && (
                          <p className="text-[11px] text-content-muted leading-snug">{agent.description}</p>
                        )}
                      </div>
                    </div>
                  </button>
                  ...
                  {isSelected ? (
                    ...
                    !isDisabled && (
                      <div className="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full border-2 border-line bg-surface-card flex items-center justify-center z-10" />
                    )
                  )}
                </div>
              );
