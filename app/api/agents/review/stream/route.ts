...
/**
 * 流式获取单个 Agent 的互评（Server-Sent Events）
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sessionId || !agentId || !roundIndex || !otherAgentsSpeeches) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: sessionId, agentId, roundIndex, otherAgentsSpeeches' }),
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // 发送完成信息
          if (!safeEnqueue(encoder.encode(`data: ${JSON.stringify({ type: 'done', agentId, agentName: agent.name, review: fullContent })}\n\n`))) {
            return;
          }
          ...
          console.error('[API /api/agents/review/stream] Error:', error);
    ...
    console.error('[API /api/agents/review/stream] Error:', error);
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : 'Failed to generate review' }),
